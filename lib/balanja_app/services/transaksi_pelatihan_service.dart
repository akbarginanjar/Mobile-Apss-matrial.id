  ...
  Future<List<Transaksi>> getTransaksiPelatihanByStatus(String status) async {
    ...
    try {
      final Response conn = await get(
        '${Base.url}/v1/transaksi-online?konsumen_member_id=${GetStorage().read('member_id')}&show_bukti_tf=1&status=$status&view_as_invoice=1&start=0&length=20&transaction_type=pelatihan',
        headers: {
          'secret':
      ...
      if (conn.statusCode == 200) {
        ...
        final errorMessage = conn.body['message'] ??
            'Gagal memuat data transaksi pelatihan. Status: ${conn.statusCode}';
